...
//                .reservedTime("20990101000000") // 예약시간 설정 yyyyMMddHHmmss
        ...
        SureMModels.ApiResponse<Void> sendResult = client.sendMessage(SureMServiceType.MMS, lmsRequest);
        System.out.printf("LMS 발송 결과: [messageId:%d / ackCode:%s]%n", messageId, sendResult.getCode());
    }
...
//                .reservedTime("20990101000000") // 예약시간 설정 yyyyMMddHHmmss
        ...
        SureMModels.ApiResponse<Void> sendResult = client.sendMessage(SureMServiceType.MMS, mmsRequest);
        System.out.printf("MMS 발송 결과: [messageId:%d / ackCode:%s]%n", messageId, sendResult.getCode());
    }
}
